...
Evaluate the incident for the target operations module: "${moduleName}".
If the user provides custom guidance or a specific follow-up question ("${userMessage || ''}"), you must directly address it while delivering the operational structure.
...
Active Incident Scenario: ${scenario}
User Additional Context/Question: ${userMessage || "None provided"}
  ...
  app.post("/api/arena-pulse/tts", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ parts: [{ text: `Say with clear, professional authority: ${text}` }] }],
        config: {
